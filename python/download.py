...
def download_progress_hook(count, block_size, total_size):
    ...
    print('\r>> Downloading... {}%'.format(percent), end='')
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.isdir(dst_dir):
            ...
            for dli in range(len(dl_files)):
                ...
                urllib.request.urlretrieve(src_file, dst_file, reporthook=download_progress_hook)
                print('>> Downloaded dataset {} archive {}...'.format(dataset, dl_file))
                print('>> Extracting dataset {} archive {}...'.format(dataset, dl_file))


                # download_command = 'wget {} -O {}'.format(src_file, dst_file)
                # print('>> Downloading dataset {} archive {}...'.format(dataset, dl_file))
                # print("Command:", download_command)  # 打印命令
...
def download_features(data_dir):
    ...
    for di in range(len(datasets)):
        ...
        if not os.path.exists(feat_dst_file):
            ...
            urllib.request.urlretrieve(feat_src_file, feat_dst_file)
